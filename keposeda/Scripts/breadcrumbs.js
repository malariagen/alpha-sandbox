		...
		else
		{
            ...
            for (i=0; i<numSegs; i++)
                if (current_label==seg_labels[i])
                    segnr = i;
            if (segnr >= 0) {//navigates to somewhere in the breadcrumb
                cookieValue = "";
                for (i=0; i<=segnr; i++)
                    cookieValue += tU + seg_URLs[i] + tUL + seg_labels[i] + tL;
                writeSessionCookie(cookieName, cookieValue);
                writeBreadcrumb(segnr);
            }
...
function writeBreadcrumb(numSegs)
{
	...
	for(var i=0; i<numSegs; i++) {
	    backcount = numSegs - i;
	    breadcrumb += '<a onclick=\"ClickCrumb('+backcount+')\" href=\"#\">' + seg_labels[i] + '</a> > '
	}
